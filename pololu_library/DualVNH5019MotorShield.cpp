...
{
  //Pin map
  //PWM1 and PWM2 cannot be remapped because the library assumes PWM is on timer1, so PWM1 is pin 9, PWM2 is pin 10
  _INA1 = INA1;
  _INB1 = INB1;
  _EN1DIAG1 = EN1DIAG1;
  _CS1 = CS1;
...
void DualVNH5019MotorShield::setM1Speed(int speed)
{
  ...
  if (speed > 255)  // Max PWM dutycycle
    speed = 255;
  //#if defined(__AVR_ATmega168__)|| defined(__AVR_ATmega328P__)
  //OCR1A = speed * (1.57); // convert from 255 max to 400 for pwm
  //#else
...
void DualVNH5019MotorShield::setM2Speed(int speed)
{
  ...
  if (speed > 255)  // Max 
    speed = 255;
  //#if defined(__AVR_ATmega168__)|| defined(__AVR_ATmega328P__)
  //OCR1B = speed * (1.57); // convert from 255 max to 400 for pwm
  //#else
...
void DualVNH5019MotorShield::setM1Brake(int brake)
{
  ...
  digitalWrite(_INB1, LOW);
  //#if defined(__AVR_ATmega168__)|| defined(__AVR_ATmega328P__)
  //OCR1A = brake * (1.57); // convert from 255 max to 400 for pwm
  //#else
...
void DualVNH5019MotorShield::setM2Brake(int brake)
{
  ...
  digitalWrite(_INB2, LOW);
  //#if defined(__AVR_ATmega168__)|| defined(__AVR_ATmega328P__)
  //OCR1B = brake * (1.57); // convert from 255 max to 400 for pwm
  //#else
